...
const dataslider = [
    {picture: taxicar, alt: 'Taxi', title: 'Taxi', 
...
const SilderSection = ({title}) => {
    ...
    const textShow = "lg:ml-20 transition-opacity duration-700 ease-in opacity-100 w-4/5 lg:w-2/5"; 
    const textHide = "lg:ml-20 opacity-0 w-4/5 lg:w-2/5";

    const dotShow = "transition-opacity transition-colors duration-500 ease-in opacity-100 text-gray-primary text-base lg:text-lg mr-3 cursor-pointer";
    const dotHide = "transition-opacity transition-colors duration-500 ease-in opacity-770 text-gray-300 text-base lg:text-lg mr-3 cursor-pointer";
        ...
        <section className="min-h-[768px]">
            <div className="container mx-auto">
                ...
                <div className="my-10 mx-2 lg:mx-20  relative h-128 lg:h-110 group">
                    <div className="text-4xl cursor-pointer p-2 hover:bg-gray-300 w-fit absolute top-48 lg:top-1/2 left-2 opacity-0 group-hover:opacity-100 group-hover:transition-all duration-300 ease-in
                    lg:left-8"
                        onClick={prevSlide} aria-hidden="true">
                            <FontAwesomeIcon icon={faChevronLeft}/>
                        </div>
                    <div className="text-4xl cursor-pointer p-2 hover:bg-gray-300 w-fit absolute top-48 lg:top-1/2 right-8 opacity-0 group-hover:opacity-100 group-hover:transition-all duration-300 ease-in
                    z-10"
                    onClick={nextSlide} aria-hidden="true">
                        <FontAwesomeIcon icon={faChevronRight}/>
                    </div>
                    {dataslider.map((data, idx) => {
                        return (
                            <div key={idx} className="absolute w-4/5 h-full left-16 lg:left-24 top-20 flex flex-col lg:flex-row justify-start">
                                ...
                                <div className={idx === index ? textShow : textHide}>
                                    <h2 className="text-3xl font-bold  mt-0">{data.title}</h2>
                                    <p className="text-base 2xl:text-lg my-2  text-justify">{data.des}</p>
                                    {data.des2 ? <p className="text-base 2xl:text-lg my-2 text-justify"> {data.des2} </p>: null}
                                </div>
                            </div>
                    ...
                    <div className="absolute bottom-1 2xl:top-105 flex justify-center w-full">
                        ...
                        return <FontAwesomeIcon icon={faCircle} key={i}  className={i===index ? dotShow : dotHide} 
                        onClick={()=>setIndex(i)} aria-hidden="true"/>
                        })}
                    </div>
