...
export const query = graphql`query ($language: String!) {
  ...
  allContentfulAsset(
    filter: {title: {in: ["pa.DrvHero.bgImage", "pa.DrvWhyDiDi.image", "pa.DrvFeatures.image"]}}
  ) {
    nodes {
      id
      title
  ...
  allContentfulProduct(
    filter: {country: {elemMatch: {code: {eq: "pa"}}}, category: {eq: "driver"}}
  ) {
    nodes {
      name
      description
  ...
  allContentfulCity(filter: {country: {code: {eq: "pa"}}}, sort: {name: ASC}) {
    nodes {
      name
      slug
      image {
