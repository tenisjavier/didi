...
export const query = graphql`query ($language: String!) {
  ...
  allContentfulAsset(
    filter: {title: {regex: "/(do.FoodBusinessHero.bgImage)|(do.FoodBusinessColumns.image)|(do.FoodBusinessDownloads.image)/"}}
    sort: {title: ASC}
  ) {
    nodes {
      id
  ...
  allContentfulProduct(
    filter: {country: {elemMatch: {code: {eq: "do"}}}, category: {eq: "food"}, name: {eq: "Food Business"}}
  ) {
    nodes {
      name
      phone
