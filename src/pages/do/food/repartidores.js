...
const FoodDelivery = ({ data }) => {
  ...
  const foodDeliveryHeroBgImage = images.filter((image) => {
    return image.title === "do.FoodDeliveryHero.bgImage";
  })[0];
  const foodDeliveryColumnsImages = images.filter((image) => {
    return image.title.indexOf("do.FoodDeliveryColumns.image") !== -1;
  });
...
export const query = graphql`query ($language: String!) {
  ...
  allContentfulAsset(
    filter: {title: {regex: "/(do.FoodDeliveryHero.bgImage)|(do.FoodDeliveryColumns.image)|(do.FoodDeliveryDownloads.image)/"}}
    sort: {title: ASC}
  ) {
    nodes {
      id
