...
const Food = ({ data }) => {
  ...
  const foodHeroMobileBgImage = images.filter((image) => {
    return image.title === "cl.FoodHeroMobile.bgImage";
  })[0];
...
export const query = graphql`
  query {
    allContentfulAsset(
      filter: {
        title: {
          regex: "/(cl.FoodHero.bgImage)|(cl.FoodHeroMobile.bgImage)|(cl.FoodColumns.image)|(cl.FoodBusinessCTA.image)|(cl.FoodDeliveryCTA.image)/"
        }
      }
      sort: { title: ASC }
    ) {
