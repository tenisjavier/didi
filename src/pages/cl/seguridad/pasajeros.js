...
export const query = graphql`
  query {
    allContentfulAsset(
      filter: {
        title: {
          regex: "/(mx.SafetyPax.image)|(mx.SafetyBeforePaxColumns.image)|(mx.SafetyDuringPaxColumns.image)|(mx.SafetyFinishPaxColumns.image)|(mx.PaxSafety.image)/"
        }
      }
      sort: { title: ASC }
    ) {
    ...
    allContentfulFeature(
      filter: { country: { code: { eq: "cl" } }, category: { eq: "pax" } }
    ) {
      nodes {
        name
        slug
