...
export const query = graphql`query ($language: String!) {
  ...
  allContentfulAsset(
    filter: {title: {regex: "/(cr.FoodBusinessHero.bgImage)|(cr.FoodBusinessColumns.image)|(cr.FoodBusinessDownloads.image)/"}}
    sort: {title: ASC}
  ) {
    nodes {
      id
  ...
  allContentfulProduct(
    filter: {country: {elemMatch: {code: {eq: "cr"}}}, category: {eq: "food"}, name: {eq: "Food Business"}}
  ) {
    nodes {
      name
      phone
