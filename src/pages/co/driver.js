...
export const query = graphql`query ($language: String!) {
  ...
  allContentfulAsset(
    filter: {title: {in: ["co.DrvHero.bgImage", "co.DrvWhyDiDi.image", "co.DrvFeatures.image"]}}
  ) {
    nodes {
      id
      title
  ...
  allContentfulProduct(
    filter: {country: {elemMatch: {code: {eq: "co"}}}, category: {eq: "driver"}}
  ) {
    nodes {
      name
      description
  ...
  allContentfulCity(filter: {country: {code: {eq: "co"}}}, sort: {name: ASC}) {
    nodes {
      name
      slug
      image {
